...
def run_streamlit_app():
    ...
    if state and search_term: 
        with st.spinner("Searching..."):
            ...
            with col2:
                st.markdown(f"<div style='font-size: 20px;'>Total Available PMID'S for {search_term} are  <span style='color: red; font-size: 24px;'><b> {count}</b></span></div>", unsafe_allow_html=True)
            if count > 0:
                display_only = min(count+1,display_only)
                with col3:
                    st.markdown(f"<div style='font-size: 20px;'>Displaying Top ↗️📈 <span style='color: red; font-size: 24px;'><b>{display_only-1}</b></span></div>", unsafe_allow_html=True)
